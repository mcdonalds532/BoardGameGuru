...
- [ ] Phase 6: deployment
- [~] Phase 7: eval + docs — quantified fine-tune eval done (see "Fine-Tune Eval Results" below); `run_eval.py` now implemented (smoke/integration eval hitting a live `/query` endpoint, 20/20 passed locally, avg 4.4s latency, results in `backend/eval/live_eval_results.json`, gitignored); README/final docs still open pending live deploy URLs.
